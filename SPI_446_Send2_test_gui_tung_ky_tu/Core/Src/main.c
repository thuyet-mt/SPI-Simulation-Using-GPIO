...
int main(void)
{
  ...
  char receive_char[0];
  int copy_buffer[8];
//  for (size_t i = 0; i < strlen(send_string); i++)
//  {
	  receive_char[0] = send_string[0];
	  //printf("%c", receive_char[0]);
	  for (size_t i = 0; i <=7 ; i++)
	  {
		  TogglePin();
		  copy_buffer[i]=((receive_char[0] >> i) & 1 ? 1 : 0);
		  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, copy_buffer[i]);
